...
namespace CLTFEM.Classes.Structural
{
    public class ShellElement
    {
        ...
        private int _ID;
        private Node[] _nodeList = new Node[9];
        private Node[] _dispList = new Node[9];
        ...
        public ShellElement(int ID, Node n1, Node n2, Node n3, Node n4, Node n5, Node n6, Node n7, Node n8, Node n9, double thickness, int layers, Material material)
        {
            _ID = ID;
            _nodeList[0] = n1;
            _nodeList[1] = n2;
        ...
        {
            double x = 0, y = 0, z = 0;
            for (int i = 0; i < 4; i++) //this will only run for the edge nodes
            {
                x += _nodeList[i].Point.X;
                y += _nodeList[i].Point.Y;
                z += _nodeList[i].Point.Z;
            }
        ...
        public static double ShapeFunctionN(int i, double e, double n)
        {
            ...
            switch (i)
            {
                ...
                case 8:
                    return (1.0 - e) * (1.0 + n) * (1.0 - n) / 2.0;
                default:
                    return (1.0 - Math.Pow(e, 2.0)) * (1 - Math.Pow(n, 2.0));
